...
# This part of code stablish the ip and the port of the server
...
class testHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
    # GET
    def do_GET(self):
        # This determines the send response status code
        self.send_response(200)
        #Now, the send headers code
        self.send_header('Content-type', 'text/html')
        self.end_headers()
            #An finally the send message back to client
        if self.path == "/":
